                ...
                match sig_def.members.get(m.name) {
                    some(scheme) => {
                        let member_def_id = match scheme.def_id {
                            some(id) => id,
                            none => panic(
                                "unreachable: registered sig member has no exact DefId")
                        }
                        hmembers.push(HSigMember { name: m.name,
...
fn check_effect_decl(mut ctx: InferCtx, name: Str, type_params: List<TypeParam>, ast_ops: List<EffectOpDecl>, is_pub: Bool, span: Span) -> HDecl {
    ...
    for op in def.ops {
        ...
        hops.push(HEffectOp { name: op.name, def_id: op.def_id,
                                    ...
                                    match tm.ty {
                                        Type::FnType { params: trait_params,
                                            ...
                                            while pi < trait_params.len() {
                                                ...
                                                hparams.push(HParam { name: pname,
                                                    ty: param_ty, def_id: some(pid),
                                                    is_mutable: p_is_mut,
                                                    ownership_mode: PARAM_OWNERSHIP_UNKNOWN })
...
fn check_trait_decl(mut ctx: InferCtx, name: Str, type_params: List<TypeParam>, ast_methods: List<Decl>, is_pub: Bool, span: Span) -> HDecl {
    ...
    for m in trait_def.methods {
        ...
        hmethods.push(HTraitMethod { name: m.name, def_id: m.def_id,
...
fn check_extern_fn_decl(
    ...
    for p in params {
        ...
        hparams.push(HParam { name: p.name, ty: ptype,
    ...
    for p in params {
        ...
        match param_scheme {
            some(ps) => {
                ...
                hparams.push(HParam { name: p.name, ty: ptype,
                    def_id: ps.def_id, is_mutable: p.is_mutable,
                    ownership_mode: PARAM_OWNERSHIP_UNKNOWN })
            },
            none => hparams.push(HParam { name: p.name, ty: ptype,
                def_id: none, is_mutable: p.is_mutable,
                ownership_mode: PARAM_OWNERSHIP_UNKNOWN })
        }
...
fn check_registered_body(
    ...
    let mut effect_op_identities: Map<Str, Int> = map_new()
    let mut effect_entries = ctx.env.types.effects.entries()
    effect_entries.sort_by(compare_by_first)
    for effect_entry in effect_entries {
        let (_, effect_def) = effect_entry
        for op in effect_def.ops {
            let key = exact_effect_op_key(effect_def.name, op.name)
            match effect_op_identities.get(key) {
                some(existing) => if existing != op.def_id {
                    panic("unreachable: canonical effect operation registration differs")
                },
                none => effect_op_identities.insert(key, op.def_id)
            }
        }
    }
