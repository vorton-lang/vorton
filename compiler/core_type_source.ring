...
// One source-less physical Core type backs every explicit EffectCtx pointer.
// Semantic entries/layouts remain in effect_contract; this fact exists only
// for Flow/ResourcePlanner Borrow/Own/all-exit cleanup.
pub struct CoreEffectCtxTypeSource { aggregate_fact: CoreTypeFactRef }
pub fn make_core_effect_ctx_type_source(
    aggregate_fact: CoreTypeFactRef
) -> CoreEffectCtxTypeSource {
    CoreEffectCtxTypeSource { aggregate_fact: aggregate_fact }
}
pub fn core_effect_ctx_source_aggregate_fact(
    value: CoreEffectCtxTypeSource
) -> CoreTypeFactRef { value.aggregate_fact }
pub fn core_effect_ctx_type_source_same(
    left: CoreEffectCtxTypeSource, right: CoreEffectCtxTypeSource
) -> Bool { core_type_fact_same(left.aggregate_fact, right.aggregate_fact) }
