...
use types::{Type, EffectRow, StructField, EnumVariant, RecordField,
    ...
    EMPTY_ROW, callable_ownership_term,
    shadow_callable_param_ownership,
    callable_owning_transfer_levels,
    validate_shadow_ownership_metadata}
...
pub struct HParam {
    ...
    pub def_id: Int?,
    pub is_mutable: Bool,
    // S1 shadow-only caller/callee mode.  Unknown is inert until activation.
...
pub struct HProgram {
    ...
    // S1 shadow-only; Perceus/verifier/codegen remain non-consumers.
...
fn validate_hir_params(
    ...
    for param in params {
        if param.ownership_mode < 0 ||
           param.ownership_mode > PARAM_OWNERSHIP_UNKNOWN {
            panic("HIR ${label} parameter '${param.name}' has invalid shadow ownership mode")
        }
...
fn required_shadow_term(
    metadata: OwnershipMetadata, def_id: Int, label: Str
) -> Int {
    match metadata.callable_by_def_id.get(def_id) {
        some(term) => term,
        none => panic("HIR ${label} DefId ${def_id} has no shadow callable metadata")
    }
}

fn validate_hir_shadow_callable_type(
    metadata: OwnershipMetadata, def_id: Int, ty: Type, label: Str
) {
    let type_term = match callable_ownership_term(ty) {
        some(term) => term,
        none => panic("HIR ${label} DefId ${def_id} is not callable-typed")
    }
    let metadata_term = required_shadow_term(metadata, def_id, label)
    if type_term != metadata_term {
        panic("HIR ${label} DefId ${def_id} callable term differs from metadata")
    }
    let state = match metadata.callable_state_by_def_id.get(def_id) {
        some(value) => value,
        none => panic("HIR ${label} DefId ${def_id} has no shadow state")
    }
    let expected_levels = callable_owning_transfer_levels(metadata, ty)
    if state.transfer_levels.len() != expected_levels.len() {
        panic("HIR ${label} DefId ${def_id} transfer spine depth differs from type")
    }
    let mut level_index = 0
    while level_index < expected_levels.len() {
        match (state.transfer_levels.get(level_index),
               expected_levels.get(level_index)) {
            (some(actual), some(expected)) => {
                if actual.ownership_term != expected.ownership_term ||
                   actual.force_params.len() != expected.force_params.len() {
                    panic("HIR ${label} DefId ${def_id} transfer spine differs from type")
                }
            },
            _ => panic("HIR ${label} DefId ${def_id} transfer spine is incomplete")
        }
        level_index = level_index + 1
    }
    match state.producer_def_id {
        some(producer) => if !metadata.callable_by_def_id.contains_key(producer) {
            panic("HIR ${label} DefId ${def_id} producer has no metadata")
        },
        none => {}
    }
}

fn validate_hir_shadow_params(
    metadata: OwnershipMetadata, owner_def_id: Int,
    params: List<HParam>, label: Str
) {
    let owner_term = required_shadow_term(metadata, owner_def_id, label)
    let mut index = 0
    for param in params {
        let expected_mode = shadow_callable_param_ownership(
            metadata, owner_term, index)
        if param.ownership_mode != expected_mode {
            panic("HIR ${label} parameter '${param.name}' ownership mode differs from metadata")
        }
        match (param.def_id, param.ty) {
            (some(def_id), Type::FnType { .. }) =>
                validate_hir_shadow_callable_type(
                    metadata, def_id, param.ty,
                    "${label} parameter '${param.name}'"),
            _ => {}
        }
        index = index + 1
...
fn validate_hir_shadow_owner(
    metadata: OwnershipMetadata, def_id: Int,
    params: List<HParam>, return_type: Type, effects: EffectRow,
    label: Str
) {
    let term = required_shadow_term(metadata, def_id, label)
    let mut param_types: List<Type> = []
    for param in params { param_types.push(param.ty) }
    validate_hir_shadow_callable_type(metadata, def_id,
        Type::FnType {
            params: param_types, return_type: return_type,
            effects: effects, ownership_term: term
        }, label)
    validate_hir_shadow_params(metadata, def_id, params, label)
}

fn validate_hir_shadow_binding(
    metadata: OwnershipMetadata, def_id: Int?, ty: Type, label: Str
) {
    match (def_id, ty) {
        (some(id), Type::FnType { .. }) => {
            if (is_synthetic_anf_def_id(id) ||
                is_synthetic_rc_def_id(id)) &&
               !metadata.callable_by_def_id.contains_key(id) {
                return
            }
            validate_hir_shadow_callable_type(metadata, id, ty, label)
        },
        _ => {}
    }
}

fn validate_hir_shadow_arm(
    arm: HMatchArm, metadata: OwnershipMetadata
) {
    for binding in arm.bindings {
        validate_hir_shadow_binding(
            metadata, some(binding.def_id), binding.ty,
            "pattern binding '${binding.name}'")
    }
    match arm.guard {
        some(guard) => validate_hir_shadow_expr(guard, metadata),
        none => {}
    }
    validate_hir_shadow_expr(arm.body, metadata)
}

fn validate_hir_shadow_stmt(stmt: HStmt, metadata: OwnershipMetadata) {
    match stmt {
        HStmt::Let { name, def_id, ty, init, .. } => {
            validate_hir_shadow_binding(
                metadata, def_id, ty, "let '${name}'")
            validate_hir_shadow_expr(init, metadata)
        },
        HStmt::Var { name, def_id, ty, init, .. } => {
            validate_hir_shadow_binding(
                metadata, def_id, ty, "var '${name}'")
            validate_hir_shadow_expr(init, metadata)
        },
        HStmt::Assign { target, value, .. } => {
            validate_hir_shadow_expr(target, metadata)
            validate_hir_shadow_expr(value, metadata)
        },
        HStmt::ExprStmt { expr, .. } =>
            validate_hir_shadow_expr(expr, metadata),
        HStmt::Return { value, .. } => match value {
            some(expr) => validate_hir_shadow_expr(expr, metadata),
            none => {}
        },
        HStmt::While { condition, body, .. } => {
            validate_hir_shadow_expr(condition, metadata)
            validate_hir_shadow_expr(body, metadata)
        },
        HStmt::ForIn { iterable, body, .. } => {
            validate_hir_shadow_expr(iterable, metadata)
            validate_hir_shadow_expr(body, metadata)
        },
        HStmt::LetDestructure { bindings, init, .. } => {
            for binding in bindings {
        ...
        HStmt::IfLet { bindings, expr, then_block, else_block, .. } => {
            ...
            validate_hir_shadow_expr(expr, metadata)
            validate_hir_shadow_expr(then_block, metadata)
            match else_block {
                some(block) => validate_hir_shadow_expr(block, metadata),
                none => {}
            }
        },
        HStmt::Break { .. } | HStmt::Continue { .. } |
...
fn validate_hir_shadow_expr(expr: HExpr, metadata: OwnershipMetadata) {
    match expr {
        HExpr::Ident { def_id, ty, .. } =>
            validate_hir_shadow_binding(metadata, def_id, ty, "identifier"),
        HExpr::BinOp { left, right, .. } => {
            validate_hir_shadow_expr(left, metadata)
            validate_hir_shadow_expr(right, metadata)
        },
        HExpr::UnaryOp { operand, .. } =>
            validate_hir_shadow_expr(operand, metadata),
        HExpr::Call { callee, callee_def_id, callable_result_def_id,
                      args, ty, .. } => {
            match callee_def_id {
                some(def_id) => validate_hir_shadow_callable_type(
                    metadata, def_id, hexpr_type(callee), "call callee"),
                none => {}
            }
            match ty {
                Type::FnType { .. } => match callable_result_def_id {
                    some(def_id) => validate_hir_shadow_callable_type(
                        metadata, def_id, ty, "call result"),
                    none => panic("HIR callable result has no exact DefId")
                },
                _ => if callable_result_def_id.is_some() {
                    panic("HIR non-callable result carries a callable DefId")
                }
            }
            validate_hir_shadow_expr(callee, metadata)
            for arg in args { validate_hir_shadow_expr(arg, metadata) }
        },
        HExpr::FieldAccess { receiver, .. } =>
            validate_hir_shadow_expr(receiver, metadata),
        HExpr::StructLit { fields, spread, .. } => {
            for field in fields {
                validate_hir_shadow_expr(field.value, metadata)
            }
            match spread {
                some(value) => validate_hir_shadow_expr(value, metadata),
                none => {}
            }
        },
        HExpr::NamedVariantConstruct { fields, spread, .. } => {
        ...
        HExpr::MatchExpr { scrutinee, arms, .. } => {
            validate_hir_shadow_expr(scrutinee, metadata)
            for arm in arms { validate_hir_shadow_arm(arm, metadata) }
        },
        HExpr::TryCatch { body, arms, .. } => {
            validate_hir_shadow_expr(body, metadata)
            for arm in arms { validate_hir_shadow_arm(arm, metadata) }
        ...
        HExpr::Block { stmts, tail, .. } => {
            for stmt in stmts { validate_hir_shadow_stmt(stmt, metadata) }
            match tail {
                some(value) => validate_hir_shadow_expr(value, metadata),
                none => {}
            }
        ...
        HExpr::IfExpr { condition, then_branch, else_branch, .. } => {
            validate_hir_shadow_expr(condition, metadata)
            validate_hir_shadow_expr(then_branch, metadata)
            match else_branch {
                some(value) => validate_hir_shadow_expr(value, metadata),
                none => {}
            }
        ...
        HExpr::HandleExpr { body, handlers, .. } => {
            validate_hir_shadow_expr(body, metadata)
            for handler in handlers {
                let op_term = required_shadow_term(
                    metadata, handler.op_def_id, "effect handler operation")
                let mut index = 0
                for param in handler.params {
                    if param.ownership_mode != shadow_callable_param_ownership(
                            metadata, op_term, index) {
                        panic("HIR effect handler parameter mode differs from operation metadata")
                    }
                    validate_hir_shadow_binding(
                        metadata, param.def_id, param.ty,
                        "effect handler parameter '${param.name}'")
                    index = index + 1
                }
                match handler.resume_binding {
                    some(binding) => validate_hir_shadow_binding(
                        metadata, some(binding.def_id), binding.ty,
                        "handler resume '${binding.name}'"),
                    none => {}
                }
                validate_hir_shadow_expr(handler.body, metadata)
            }
        },
        HExpr::Lambda { def_id, params, return_type, body, ty, .. } => {
            validate_hir_shadow_callable_type(
                metadata, def_id, ty, "lambda")
            validate_hir_shadow_params(metadata, def_id, params, "lambda")
            validate_hir_shadow_expr(body, metadata)
        },
        HExpr::EffectOp { op_def_id, args, .. } => {
            let _ = required_shadow_term(
                metadata, op_def_id, "effect operation")
            for arg in args { validate_hir_shadow_expr(arg, metadata) }
        },
        HExpr::RangeExpr { start, end, .. } => {
            validate_hir_shadow_expr(start, metadata)
            validate_hir_shadow_expr(end, metadata)
        },
        HExpr::ListLit { elements, .. } => {
            for element in elements {
                validate_hir_shadow_expr(element, metadata)
            }
        },
        HExpr::TupleLit { elements, .. } => {
            for element in elements {
        ...
        HExpr::IndexExpr { receiver, index, .. } => {
            validate_hir_shadow_expr(receiver, metadata)
            validate_hir_shadow_expr(index, metadata)
        },
        HExpr::Clone { inner, .. } =>
            validate_hir_shadow_expr(inner, metadata),
        HExpr::ReturnExpr { value, .. } => match value {
            some(inner) => validate_hir_shadow_expr(inner, metadata),
            none => {}
        },
        HExpr::UnsafeBlock { body, .. } =>
            validate_hir_shadow_expr(body, metadata),
...
fn validate_hir_shadow_decls(
    decls: List<HDecl>, metadata: OwnershipMetadata
) {
    for decl in decls {
        match decl {
            HDecl::Fn { name, def_id, params, return_type, effects, body, .. } => {
                let exact_def_id = required_hir_def_id(
                    def_id, "function '${name}'")
                validate_hir_shadow_owner(
                    metadata, exact_def_id, params,
                    return_type, effects, "function '${name}'")
                validate_hir_shadow_expr(body, metadata)
            },
            HDecl::ExternFn { name, def_id, params, return_type, effects, .. } => {
                let exact_def_id = required_hir_def_id(
                    def_id, "extern function '${name}'")
                validate_hir_shadow_owner(
                    metadata, exact_def_id, params,
                    return_type, effects, "extern function '${name}'")
            },
            HDecl::Const { name, def_id, ty, init, .. } => {
                let exact_def_id = required_hir_def_id(
                    def_id, "const '${name}'")
                let term = required_shadow_term(
                    metadata, exact_def_id, "const getter '${name}'")
                validate_hir_shadow_callable_type(
                    metadata, exact_def_id,
                    Type::FnType {
            ...
            HDecl::Impl { methods, .. } =>
                validate_hir_shadow_decls(methods, metadata),
            HDecl::ModBlock { decls, .. } =>
                validate_hir_shadow_decls(decls, metadata),
            HDecl::Effect { name, ops, .. } => {
                for op in ops {
                    validate_hir_shadow_owner(
                        metadata, op.def_id, op.params,
                        op.return_type, EMPTY_ROW,
                        "effect operation '${name}.${op.name}'")
                    match op.default_body {
                        some(body) => validate_hir_shadow_expr(body, metadata),
                        none => {}
                    }
                }
            },
            HDecl::Trait { name, methods, .. } => {
                for method in methods {
                    validate_hir_shadow_owner(
                        metadata, method.def_id, method.params,
            ...
            HDecl::Sig { name, members, .. } => {
                for member in members {
                    validate_hir_shadow_callable_type(
                        metadata, member.def_id, member.fn_type,
                        "sig member '${name}.${member.name}'")
                }
            },
            HDecl::Test { body, .. } =>
