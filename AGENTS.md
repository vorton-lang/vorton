...
- 当前 tree 的唯一 compiler implementation authority 是 `crates/vorton-compiler`；library 同时保留单 source `parse` 与纯内存项目 `resolve_project` 入口；根 Rust workspace 固定使用 Rust `1.98.0`，真实 compiler gate 由 Governance CI 执行。
