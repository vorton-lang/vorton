...
- 语言公理见 [`docs/philosophy.md`](docs/philosophy.md)，稳定设计见 [`docs/design.md`](docs/design.md)，用户规范见 [`docs/lang-spec/`](docs/lang-spec/)。GitHub 模板和标签入口见 [`docs/workflow.md`](docs/workflow.md)。

## 当前项目事实

- Vorton 当前以 Rust 重建编译器，先闭合 `source → token → AST → diagnostic`，再按 Issue 推进 checker、IR、ownership、C11 后端与 CLI。
