...
限定 path 可跨 file/inline 边界。路径首先冻结 lexical/nominal root；field、method 与 associated item 等仍依赖类型信息的后续选择由 Checker 完成，Resolver 不按 leaf spelling 或大小写伪造 target。
...
Field、method、associated item 与 effect operation 保持 owner-scoped，不自动注入普通 module scope。Module declaration、普通具名 declaration 与 import 在整个 module scope 可见，因而支持 forward reference。
