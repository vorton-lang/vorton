...
当前 `vorton_compiler::check_project` 覆盖普通函数、source trait/supertrait、inherent/trait impl、receiver method、associated function 与关联类型 assignment/default/bound。类型包括 `Int`、`Float`、`Bool`、`Unit`、`Never`、formal、tuple、普通 struct/enum、非泛型透明 alias、合法关联 projection，以及当前 shared Fn 消费范围内的具名函数值。Named-trait 形成条件、where 的 nominal/tuple/projection subject、conformance、orphan、overlap 与方法可见性使用同一选择事实；alias/re-export 保留原 LibraryId。Private representation 可以运输，公开签名、bound、关联结果与 effect 不得泄露 private declaration。
...
表达式包括现有 literal、参数/不可变 local 引用、tuple、block、if/else、简单 let、return、普通/方法/间接调用、无 spread 的 struct 与 enum 构造、Copy 字段读取和字段借用。Primitive comparison 与显式 source core impl 的 comparison 使用 exact core evidence。函数值只接受具名 provider 的实参、本地不可变别名及 shared Fn 调用；同检查单元的 provider 必须有 closed header。`F: Fn + fn(...)` 和已有 Fn evidence 确定为 Borrow 的 `call F` 支持显式及省略 shape row；参数/返回/mode 保持 invariant，callback 的实际公开 row 产生 effect actual 下界。

Effect 支持 system/handled/fail/mut/unsafe、alias、显隐 formals、method application、完整销毁和 selected-call 关系。有显式上界 B 时验证 A ⊆ B 并发布 B；trait/impl 的 A/I/B 使用[既定规则](traits.md#按-impl-关联的-effect-scheme)。Effect actual 取同一次调用的全部 callback 下界与各 row 合法条件的联合最小解；单条 shape 含多个 formal 不单独构成歧义。两个 callback 共用 E 时合并实际 rows，pure actual 得到空 row，payload 冲突或没有唯一最小解时拒绝。词法 unsafe 需要 module 显式授权，只消除 unsafe；module ceiling 与 main 的用户 handled-effect 逃逸规则仍检查真实公开 row。

Source/contract handled instance 使用 effect owner 的 named-trait 条件。Effect alias 展开即使消去了某个 type actual，也仍检查该 actual 的形成条件和 alias owner 条件；检查使用完成 conformance 后的 callable 输入域，不给 caller 暗增要求。

整值使用保留 live/moved/maybe-moved、借入/拥有状态与待移交 temporary。正常 scope/return、fail.raise，以及不能证明无退出的调用都贡献必要清理；仅含 handled atom 的调用也可能因外层 handler arm 失败而退出。后续实参求值失败时，前面已求值的 Move temporary 仍归 caller；进入 callee 后则由 callee 接管。Borrow 的 owner 不移交，block 的局部借用结果保活到消费调用结束。泛型清理保留 full_destruction(T)，具体受支持成员归约；unused formal、已知 empty variant、已移交值和 failure payload 不凭空增加销毁。普通 nominal 不因字段全为 Copy 而自动具有 Copy。

Format-1 支持 ordinary/trait/impl method callable record、独立 receiver selector、outer/own formal、Self、参数/返回、固定 Borrow/Move、确定 Borrow 的 callable_use，以及当前 Trait/Effect 条款。Partial records 先合组；requirements 保持未选择、[]、P 三态，外层形成条件不被删除。Source/contract 泛型按 owner、kind、ordinal 和实际关系显式对应，允许 alpha 改名；契约不能把未标注 identity 静默特化为 concrete 类型。其它属性双方显式不同且没有既定差异政策时明确 Unsupported。详见[contract 支持说明](../contract-format.md)。

Handler/catch、pattern/loop、捕获 closure、一般函数值返回/存储、FnMut/FnOnce 入口、scoped、Mut/assignment/partial move、用户 Drop/Rc/Weak、trait-derived Copy、Str/List/Range/Ptr、generic alias、FFI、生成器和后端尚未开放。成功的 opaque `CheckedProject` 保存闭合 scheme、selection/evidence、唯一 mapping、typed body 与清理义务；合法 formal/projection/method/destruction 关系可以保留，raw metavariable、待选 impl 或未定公开 mode 不得保留。
