...
当前 `vorton_compiler::check_project` 闭合普通 module／inline-module 函数的受限 HM 子集。类型包括 `Int`、`Float`、`Bool`、`Unit`、`Never`、formal、tuple、普通 struct/enum 应用，以及可展开到同一集合的非泛型 type alias。名义类型按 exact declaration identity 与 actual 区分；声明字段保留 owner formal，`Self` 指向该 owner 的同一次类型应用。函数可声明无 bound 的 type parameters；内部函数可省略参数和返回类型并由 body 推断、泛化。Metavariable、声明 formal 和已发布 scheme 在内部保持不同身份，成功的 `CheckedProject` 不保留未解 metavariable。`Str`、`List`/`Range`/`Ptr`、generic alias、associated type、callable shape、非空 Trait bound 和 effect formal 仍稳定返回 `Unsupported`。
...
普通局部绑定保持同一个 monotype。这个入口继续支持 literal、参数／不可变 local 引用、顺序 shadowing、tuple 构造和 concrete ordinal projection、block、if/else、简单 `let`、expression statement、return、本文定义的 primitive 运算与 exact ordinary-function direct call。还支持无 spread 的 struct 构造、无 payload／位置 payload／命名 payload 的 enum 构造，以及普通 struct 的字段读取和借用。构造成员按源码顺序求值，保留真实 constructor、field occurrence 和 nominal actual；字段选择消费同一递归组的类型约束。Option、Ordering 的声明和构造使用指定 core 的真实 identity，core 的 Trait／迭代协议仍只保留原有 Resolver profile。已解析但不在这组 expression／statement carrier 内的表面仍返回 `Unsupported`。

无 bound 的 generic formal 与普通 nominal 均不默认具有 `Copy`；字段全为 Copy 或为空也不会让整个 nominal 自动变成 Copy。Checker 对当前子集保留 whole-binding 的 live／moved 状态：完整返回、`let` 转移和 Move 参数调用会转移 owner；互斥分支可各自完整转移一次，顺序重复转移、移交后使用和借入值变成 owned 结果会拒绝。只借用或所有正常出口都已完整移交的 generic owner 可形成 Pure 结果。

没有用户 Drop 且实际遗留成员的清理已证明为空时，局部 nominal 与 temporary 也可纯清理；未使用的类型参数、已知无 payload 的 variant 不凭空产生 `D(T)`。构造尚未完成就提前返回时，先前成员 temporary 的义务仍须检查。Copy 字段可重复读取，non-Copy 字段只允许借用 consumer，不能经 binding、返回或 Move 调用获得拥有值。需要未知 `D(T)`、partial move 或 Copy evidence 的路径返回 `Unsupported`。递归名义引用保持有限类型边，借用或整值转发不展开字段；无法证明的实际清理义务仍明确拒绝。现有 concrete scalar 及全元素 concrete-Copy tuple 保持 Copy 行为。

Source 与 format-1 contract 的 generic function 必须都显式声明同一 formal 数量；已选择的参数／返回条款通过 exact target、declaration binder、type kind、ordinal 和使用关系对齐，未选择的条款无需重复整份 signature。Formal 显示名可以 alpha 改名。Contract 不能把源码推断出的 generic 关系用 concrete type 单态化。`generic_requirements` 未选择和显式 `[]` 继续保持不同输入事实，非空 requirement 尚未支持。实际导出的 public 输入类型和 mode 逐 position 由 source 或所属 contract 明确；private 输入、返回类型和 empty effect 可继续推断。函数 contract 的 nominal reference 按真实库、声明类别和 arity 绑定，actual 与 source 类型共同归一化；alias 与 re-export 不产生新类型。公开参数／返回、pub field 与 public enum payload 不泄露 private nominal；public struct 的 private representation field 可包含 private 类型。

省略 source effect header 或显式 `with {}` 只有在 body operation 和 exact direct-call graph 都落在上述纯子集，且所有正常出口遗留 owner 和 temporary 的清理已证明为空时才形成 empty row。`CheckedProject` 内部保留 closed scheme、调用 mapping、名义声明／actual／构造／字段 identity、normalized type、数值、exact callee、mode、empty effect、binding/use origin 和每个函数唯一的 typed body，但不公开可编辑 identity／通用查询接口，也不表示完整接口 S 或最终 `TypedHIR`。
