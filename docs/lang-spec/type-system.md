...
Effect 支持 system/handled/fail/mut/unsafe、alias、显隐 formals、method application、完整销毁和 selected-call 关系。有显式上界 B 时验证 A ⊆ B 并发布 B；trait/impl 的 A/I/B 使用[既定规则](traits.md#按-impl-关联的-effect-scheme)。两个 callback 共用 E 时合并实际 rows，pure actual 得到空 row，payload 冲突或没有唯一最小解时拒绝。词法 unsafe 需要 module 显式授权，只消除 unsafe；module ceiling 与 main 的用户 handled-effect 逃逸规则仍检查真实公开 row。
