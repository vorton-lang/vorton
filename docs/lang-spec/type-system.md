...
具名函数值可作为实参或不可变 local alias；同检查单元 provider 必须有闭合 header。`F: Fn + fn(...)` 和已知 Fn evidence 的 `call F` 使用固定 Borrow 入口。Callback shape 的 actual row 为 effect formal 提供下界，共享 formal 取唯一合法最小合并；省略 row 的独立 shape occurrence 不被合并，冲突 payload 拒绝。借入 callback 不接管原 owner；一般函数值返回/存储、捕获、scoped、FnMut/FnOnce 消费入口仍未开放。
...
Source 与 format-1 callable contract 的自身 type binder 双边显式且按 owner/ordinal/使用关系对应，允许 alpha 改名；outer binder 不重复声明成 method binder。Receiver 有独立 selector。Partial records 共享变量并比较归约后的类型；`generic_requirements` 未选择、`[]` 和 `P` 保持三态，不能暗增输入域。支持的 Effect 条款与 type/mode 选择在对应约束消费前接入，source 显式类型冲突硬错误；其它双边显式属性不同且没有差异政策时为 `Unsupported`。公开输入 type/mode 保持逐 position 明确，public signature、bound、关联结果和 effect 不泄露 private declaration；private representation 的既有规则保持。
