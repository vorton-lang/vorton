...
当前 `vorton_compiler::check_project` 联合闭合 ordinary function、source Trait/impl、具名 receiver method 与 associated function。类型包括 `Int`、`Float`、`Bool`、`Unit`、`Never`、formal、tuple、普通 struct/enum、非泛型透明 alias、合法 associated projection 和本期具名函数值。名义形成条件、supertrait、associated default/assignment/bound、关联绑定和一阶 `where` 合取消费同一套 exact owner 与 trait evidence；outer、method formal 和 `Self` 不混同。内部函数保留合法 HM 推断，普通 `let` 保存同一次求值的 monotype。
...
具名函数值可作为实参或不可变 local alias；同检查单元 provider 必须有闭合 header。`F: Fn + fn(...)` 和已知 Fn evidence 的 `call F` 使用固定 Borrow 入口。Callback shape 的 actual row 为 effect formal 提供下界；先统一同一调用 mapping 中的 payload/type actual，再据此取唯一合法最小余量及共享 formal 的合并。新的类型约束不能留下已经被固定 head 覆盖的多余 effect；省略 row 的独立 shape occurrence 不被合并，冲突 payload 拒绝。借入 callback 不接管原 owner；一般函数值返回/存储、捕获、scoped、FnMut/FnOnce 消费入口仍未开放。

整值 use 检查保留 live/moved/maybe-moved owner、正常 scope/return 与可能 failure 的清理点。后续实参失败时，先前 Move actual 仍由 caller temporary 持有；进入 callee 后其 Move 参数由 callee 接管。Failure payload 和正常返回的完整移交不重复清理。泛型遗留 owner 形成 `full_destruction(T)`；受支持 concrete actual 按成员事实归约，phantom、已知空 variant 和空成员保持精度。递归普通数据按有限销毁依赖闭包处理。该关系不是自由 effect formal，也不是单个 `Drop::drop` scheme；本期不生成物理 cleanup 指令或 CFG。

Source 与 format-1 callable contract 的自身 type binder 双边显式且按 owner/ordinal/使用关系对应，允许 alpha 改名；outer binder 不重复声明成 method binder。Receiver 有独立 selector。Partial records 共享变量并比较归约后的类型；`generic_requirements` 未选择、`[]` 和 `P` 保持三态，不能暗增输入域。支持的 Effect 条款与 type/mode 选择在对应约束消费前接入，source 显式类型冲突硬错误；其它双边显式属性不同且没有差异政策时为 `Unsupported`。公开输入 type/mode 保持逐 position 明确，public signature、bound 的 positional/associated actual、impl 关联结果、callback shape 参数/返回和 effect 不泄露 private declaration；透明 alias 在归约前仍按原声明检查可见性；private representation 的既有规则保持。

成功的 opaque `CheckedProject` 保存闭合 scheme、bodyless signature、typed body、精确 selection/evidence、调用 mapping 和清理义务，不能含 raw metavariable 或待选调用。它没有新增公开查询 API，也不是完整 S/TypedHIR。Handler/catch、pattern/循环、Mut 参数/assignment/partial move、用户 Drop、Rc/Weak、trait-derived Copy、Str/List/Range/Ptr、generic alias、FFI、生成器与完整 core/std 均在当前支持边界之外，遇到相应 source/contract 请求明确返回 `Unsupported`。
...
“可用”包括目标选择前的 header/where 资格证明。已闭合且证明不成立的条件排除对应候选；尚未完成的查询、循环或求解上限不能充当不适用证据。目标一旦选定，参数、mode 或 Effect 检查失败仍产生诊断，不改选另一个方法。关联类型选择同样先检查候选的资格与可见性。
