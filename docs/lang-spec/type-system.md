...
Language type/trait 在 Type namespace 中不可被 source declaration、import、re-export 或 generic binder 覆盖。它们不是词法关键字，不限制 Value 或 Effect namespace 的同名 binding。
...
Generic parameter 在所属 declaration 的 bounds、signature 与 body 全部可见；trait/impl 外层 generic 也对 member 可见。同一 parameter table 不得重复，内层 generic 不得遮蔽仍可见的外层 generic。Generic 可以遮蔽普通 module Type binding，但不能遮蔽 Language Type/Trait。

`Self` 是 struct、enum、trait 与 impl owner scope 中的特殊 Type identity，并由内部 method 与 closure 继承。它分别表示当前 nominal、trait 的实现者或 impl target，不是全局 Language builtin，也不是 lexer keyword。普通 Type declaration/module/generic binder 不能占用 `Self`；owner scope 外使用 `Self` 报错。Substitution、associated selection 与 impl/coherence 仍由 Checker 完成。
