...
当时顺序曾改为 `I′ -> A′ atomic transfer authority -> S′ cleanup -> final acceptance`。A′ 仍按既有用户批准一次贯通全 executable-HDecl inventory、transitive may-Drop、callable ownership modes、CFG/显式 `Take`/source-clear，并由 checker→HIR→Perceus→verifier→codegen 共用；不能缩成 Option ctor 特判、删除 ANF Drop、runtime dup、fresh-only ANF sink、旧 `56a` 双遍历、unity 或资源加码。transfer canary 先覆盖 literal/named Resource，struct/variant/list/tuple/range/slot generic owning sink、non-Drop control，以及删除 Take/错 DefId/重复 Drop mutations；Trait-default executable-HDecl 必须原子纳入。只有该矩阵绿，才恢复 exact-none W4/exit、tail negatives、shadow/loop/catch 与 S′ fixed-point/full acceptance。该重排只恢复已批准 ownership/safety 契约，不改变公开语义或保证，也不申请豁免；其实现分层现已由下述 FlowIR 架构取代。

**FlowIR single Resource Planner architecture approval（2026-08-22；2026-08-23 naming clean break）**：用户在完整讲解current pipeline、ResolvedAST/TypedHIR/CoreHIR/FlowIR/RcIR边界、隐式行为展开时点、SymbolRef identity、统一slot-state machine与certificate proof后明确表示理解并批准执行。原 `FinalHIR`/`RcHIR` 名称clean break为 `FlowIR`/`RcIR`，不保留alias。本段supersede `I′ -> A′ -> S′` 的实现分层：I′ identity-only durable claim保持；A′ source transfer与S′ exact-none/overwrite/exit cleanup由FlowIR上的唯一project-wide ResourcePlanner同一次完成。FlowIR前必须完成全部semantic lowering、typed identity、ownership-neutral ANF、pattern projection与scope-result造槽，并冻结所有binder/node/edge；Planner按Logical OwnershipShape、Physical RcShape、callable least fixed point、per-body CFG的固定顺序输出binder不变且Clone/Take/Drop/Cleanup全显式的RcIR与ranked certificate。Verifier只核验frozen constraints、有限derivation与路径守恒，codegen只接受verified RcIR；planner后无独立Perceus、无producer/sink猜测、无新增binder。
