...
当前 HM 泛型合并先由 caller 登记同一声明域内相互独立的 formals，再调用 compiler 实际消费的等价类核心。核心成功时只合并请求涉及的两个既有类并保持全部已登记独立关系；拒绝不改变类关系。ordinary function 与 nominal formals 已在 body 约束前登记；递归组不同 owner 间的 alpha 对应仍可合并，同一调用的 concrete actual 可以相同。未来 method outer／own 等新域必须由对应 caller 在约束前登记，不能从 owner 拼写或最终 mapping 反推独立性。
