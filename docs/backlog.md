...
> **Generic Physical RC B-min gate（2026-08-30 用户批准）**：撤销旧“禁止Ptr/extern作为generic actual”候选；#268/#269必须保留`List/Map/Option/Box<Ptr|extern>`等0.1 consumer。Outer aggregate shell与direct payload policy分轴：shell/buffer始终release（immortal`Option.none`例外），每个storage formal/field只携`NoRc/RingRc` physical policy，Logical Unique与Borrow/Own/Clone/Take仍由ONE Planner静态裁决；删除whole-value foreign cleanup veto。Hidden evidence只出现在真实retain/release/store-for-later obligation，escaping dependent shell存packed mask；identity/pass-through/HOF与无义务generic零evidence，nested aggregate只描述direct payload。C只机械branch，不得name/header sniff、function table、在本checkpoint前移monomorphization或第二solver；D+B迁Ring不在本轮前移，沿既有B-152。Certificate/verifier独立证明shell owner与payload policy两本账；验收覆盖List/Map/Option/Box的Ptr/Str/Resource/mixed/nested、create/read/replace/drop、empty/nonempty、single/project/cross-module及错mask/NoRc→RC/漏shell mutation与完整RC/ASan。Mask只服务当前shared/erased generic实现；既有单态化路线若把某个concrete实例完整特化，必须把policy常量化并消除其hidden evidence/shell mask，不把+8B carrier冻结成永久ABI，也不为此新增post item。
