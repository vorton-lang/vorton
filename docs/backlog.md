...
> **Approved one-time Python flat-source SCU crossing**：固定clean authority源码只在仓库外经disposable Python generator转换；按module拓扑拼接为一个Ring source，给全部module-level declarations及其真实global references分配全局唯一identifier，展开/删除internal import/alias/re-export并生成唯一entry adapter。它只调用d06现有single-file check/build，不增加compiler SCU功能、不改tracked compiler/runtime/std/tests源码，也不触碰已关闭的generated-C bridge。

> 首次命令前必须有total/unique symbol mapping、global-use resolution、zero duplicate/unresolved internal use、entry/extern/prelude/builtin/intrinsic exact census与独立review。Generator必须scope-aware并保持locals/fields/methods/variants、strings/comments/interpolation与extern ABI不变；同一机械生成器可在build前修正，但不得删声明、改算法、绕过type/effect/ownership、修改C或建立第二authority。Script/manifest/flat source/C/bridge全部hash-pinned temp evidence且非candidate；bridge只编译untouched clean current project，只有clean second product进入fixed point/smoke。若需要module-identity仿真、compiler/resolver功能、string/ABI rewrite、tracked source修改、第二authority或预计`>=10 active h`，立即stop回用户，不自动转pub/backport/B2。
