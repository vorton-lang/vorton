...
> **0.1 public physical-layout Cut-A（2026-08-30 用户批准）**：public struct/enum的transitive physical representation nominal必须同样public/exportable；private字段名保持合法，但其nominal field/payload dependency不得更private。Provider final export census递归检查nested generic/tuple/function type并稳定source diagnostic，consumer不接收private type binding。Compiler仅将`PathOwnerValue/OriginRefValue/SlotRefValue/CalleeRefValue`四个internal enum机械设pub。正常revert `a3ac0d8f` PhysicalNominalFact/ModuleExports private-layout carrier与fixtures，禁止bridge seed、generated-C shim、name fallback、第二registry或post item。验收覆盖direct/nested/cyclic/private-field/private-enum负例、全部public正例、single/project/re-export/diamond及四枚compiler迁移；old physical carrier census=0。

> **Struct move-spread partial/open-drop gate（2026-08-30 用户批准）**：#268/#269 的 struct move spread 必须始终构造 fresh result；字段静态类型保持 `T`，Flow / ONE Planner 以 exact field path 的 `Live` / `Moved` occupancy 作为唯一初始化状态 authority。字段 exact Take 后 source slot 清为 physical empty / NULL；普通 aggregate Drop 只 Drop `Live` 字段、跳过 empty / `Moved` 字段并恰好一次 release shell。全 shareable 字段走 physical RC dup、base 保持完整后执行同一普通 Drop。禁止 whole-base MovePlace / storage reuse、`ReleaseMovedAggregate`、shell bypass、extra whole Drop、把 occupancy 编码成公开 `Option<T>` / `MaybeUninit<T>` 或 backend 重建。直接 user-`Drop` aggregate 若 spread 需要任一 partial Take，必须在 source/TypedHIR 边界稳定拒绝；不得把 partial `Self` 交给用户 destructor。验收必须覆盖 base 单次求值、override LTR 且失败前零partial move、Take后source-clear、mixed Live/Moved open drop、shareable physical RC dup、shell恰好释放一次、user-Drop负例，以及杀死未clear、double-drop和专用shell释放路径的mutation。
...
> **实施硬门**：旧d5/17/df1/37a4/a7 S1候选与sealed packet只作反例/evidence，不作seed或逐点返修；a7 one-shot已user-directed终止且inconclusive。新authority从最新main建立，先固定完整type-state pipeline、typed identity、CoreHIR elaboration closure、neutral normalization、ExecutableInventory与“Planner后零binder”structural/mutation contract；所有组件只可在同一authority group渐进checkpoint，未形成完整ResourcePlanner时不得merge行为改变到main。最终验收覆盖direct/named owning sink、struct/variant/list/tuple/range/slot、function value/HOF/factory、explicit ImplFn、derive、handled effect、reexport/diamond/extern bridge、ordinary managed generic recursive shape、loop/catch/handler、none→some overwrite、partial move与single/project parity；另以稳定负例证明source trait body、delegate与raw generic aggregate不能进入TypedHIR/Core。Mutation必须杀死surface-only node、implicit executable obligation、system→evidence、handled→HostImport或resolver/type/effect/trait/backend fallback越过CoreHIR边界。随后12 GiB source-built fixed point、完整RC/ASan/self-host/full与exact CI全绿才关闭#268/#269，并立即进入B-176/B-180性能路线。
...
> **0.1 trait / impl / private-interface visibility（2026-08-23 用户批准；2026-08-30 Cut-A收窄）**：trait是整体contract，impl block无visibility；`pub impl`、trait declaration/trait impl member的`pub`均hard-fail，只有inherent member逐项控制visibility。Public item接口与bounds不得引用更private type/trait/effect；public nominal的private field/payload也必须满足transitive physical nominal可见性。Private impl可留internal coherence registry，但外部trait impl surface要求target+trait均public，public inherent只发布pub methods。当前接受后丢弃的fake`pub`由B-199 clean break，不新增visibility identity bit或private physical registry。0.1无return-position`impl Trait`/opaque type，post-0.1由B-200按真实consumer重审。
...
**范围 / 文件**：`compiler/parser.ring`/checker按decl/impl kind区分visibility：任何`pub impl`、trait declaration与trait impl中的fn/associated type出现`pub`即单轮可修错误并建议删除；inherent impl继续逐member保留`pub`/private。Public fn/const/type/trait/effect、fields/methods及generic bounds递归检查Type/Effect/Trait可见性；public nominal即使字段private，也递归检查其physical nominal dependencies并拒绝更private declaration。`compiler/exports.ring`只发布正常public type/callable surface，不运输private layout closure；private impl保留内部，public inherent只导出pub member，trait impl只有target+trait均可见才导出。Impl-member extern由B-201整体删除，不在本项保留visibility分支。`docs/lang-spec/{syntax,traits,modules,type-system}.md`、examples/fixtures同步。
...
**验收**：`pub impl`及trait declaration/trait impl的`pub fn`/`pub type`全部稳定hard-fail并给删除建议；inherent public/private method与跨模块visibility保持。Public参数/返回/field/payload/nested generic/bound/effect正反例覆盖private leak，尤其private字段承载private nominal必须在provider报错；private-target public-trait与public-target private-trait impl只在module内部可用，不进入dependency method index；same-origin re-export不扩大visibility。Mutation杀死parser skip、visibility carrier/猜测、private-layout transport、private inherent误导出和单边public impl导出。完整C e2e/golden/structural/parity/self-compile、targeted human/LLM diagnostics、double bootstrap与tracked`dist-c`literal fixed point通过，workflow validator/exact CI全绿。
