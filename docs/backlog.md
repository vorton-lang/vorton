...
> **`Range` exact builtin name gate（2026-08-27 用户决定）**：0.1 的 `Range<T>` 是 type namespace 内建保留 nominal；range syntax、`Range<Int>` annotation 与 range `for-in` 必须携带同一 exact builtin owner。任何用户 struct/enum/extern type/type alias 或 import/re-export 若在可见 type namespace 建立 `Range` 绑定，均在对应 AstSite 稳定报 `E0207`；不得 leaf-key 覆盖、panic、混合 identity 或引入 builtin qualified spelling。`Range` 不是 lexer keyword，value/function namespace 不受影响，也不顺带保留其他 builtin 名称。验收覆盖 single/project 的直接声明、import/re-export collision，以及 range syntax/annotation/for-in exact-owner 正例。
...
> **R1 dynamic handled-evidence gate（2026-08-28 用户批准）**：为闭合#268/#269的factory/returned-lambda真实consumer，B-167的custom `HandledEffectRef` function-value纵切前移并入当前authority。Lambda创建pure；ordinary named/anonymous callable按冻结effect row持hidden evidence params且不capture定义处handler；每次调用从当前dynamic handler environment borrow exact evidence，无handler则effect外传。ABI固定为`closure env（仅indirect）→ordinary args→trait dictionaries→handled evidence`，direct/method共享其余顺序；SystemEffectRef不进evidence。Internal handler arm/re-perform object可显式持outer evidence，但不得与user closure混用。必须原子删除旧lexical capture与任何hybrid/fallback；B-168 failure/control及B-169其余工作不前移，但后续须兼容本契约。验收覆盖Handle外构造/内调用、Handle内构造/外调用不保留旧handler、nested动态换绑、双层factory、generic/recursive/cross-module HOF、dictionary+effect顺序、handler re-perform及RC/ASan。
