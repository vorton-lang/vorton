...
> **Approved one-time native inline-module SCU crossing**：固定62个compiler files中只包装61个reachable files（main + 60 dependencies），排除唯一orphan `perceus.ring`；固定DAG 433 edges、0 unresolved/cycles。Generator按dependency-first topo+lexicographic tie-break，把60个sources置于native inline `pub mod`，仅将wrapper零缩进sibling `use/pub use`改为`super::`，root entry为`fn main(){ cli::cli_main() }`。禁止重命名leaves、仿真原`$$_`identity、改strings/ABI/intrinsic或tracked source。`58075b40` donor必须补exact include list、topology、system TEMP与manifest/census/hashes后重新独立review。

> 所有generator/wrapper/C/bridge均hash-pinned temp evidence、非candidate；首次d06命令前按workflow §4.3.4固定EvidenceKey并证明60 wrappers/433 edges/import-entry rewrite totality与unexpected edit=0。Bridge只编译untouched clean current project，只有clean second product进入fixed point/smoke。H1的first falsifier是fixed d06 single-file `check`；若需要leaf/global rename、module identity/string/ABI emulation、compiler/resolver或tracked source修改、第二authority或`>=10h`，立即stop，不自动转pub/backport/B2。

> **H1 review/launch ordering override**：仅首个fixed d06 check可在runner/plan read-only review尚未terminal时按exact tuple `05acf602…/51f1763d… + F61405E6… + 9C490323… + 2E89DCB1… + D2078ED3…`并发启动。结果保持quarantine；review CLEAR与root post-run EvidenceKey复核前不得支持H1、build、bridge、fixed point、bookkeeping、下一命令或claim。Review BLOCK即无条件丢弃结果，不retry/fallback；其余命令、资源、postcondition与stop门不变。
