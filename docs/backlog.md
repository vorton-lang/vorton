...
> A3固定actual ABI17/typeid745、exact83与480-types零Fn/open-tail/schema-tail/non-direct-Drop/extern census、canonical owner/edge单射及61/61 direct order。Generated-C patch上限160 formatted LOC/50 semantic actions，只触projection helper/separate list、`check_module`显式参数与late helper/hook，unexpected diff 0且RC独立review CLEAR后才可运行。所有临时artifact hash固定且非candidate/fallback；只有clean second product进入fixed point/smoke。A3为最后一次bridge尝试：任一preflight/运行失败、缺metadata/identity、额外C/source edit或重复panic均立即stop，不允许A4，后续只在B1/B2间重新由用户裁决。
