...
> **Approved A′ ABI17 types/layout-only crossing**：固定clean d06 `ModuleExports`为17 args/17 slots/typeid745；direct dependencies保持完整exports，non-direct prior-topo entry只生成shallow projection并dup slot 0 module key、1 module prefix、5 types、13 struct field orders。Slots 2–4、6–12、14–16全部fresh empty，type aliases/effects/effect aliases/traits/impls/methods/inherent/extern/mut/fn-mut一律不运输。Helper复用既有17参数constructor与typeid745 drop，遵守4 dup+13 fresh transfer、List push dup与local drop；source namespace仍唯一由`namespace_plan`决定，不新增name fallback、alias、第二registry或permanent carrier。

> A′固定exact83 overlay与全部seed/blob/source/helper-patch/manifest/tool/output hashes；helper最多24 semantic actions且unexpected diff为0。Patched seed只编译overlay bridge，bridge只编译clean private current source，只有clean second product可进入gen2/gen3 fixed point与smoke。Patched seed/helper/overlay/bridge不commit、不进tracked anchor、不作candidate/fallback，crossing后永久删除。任何ABI mapping漂移、raw-tail复现、缺失alias/value/trait/impl/Drop、canonical collision、private83 mismatch、helper residue、额外C/source edit或其他失败都立即stop并重新请求用户决定；不得扩projection或复用旧授权。
