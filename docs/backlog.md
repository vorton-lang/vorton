...
2. **#268/#269 信任闭环**：ownership/RC fixed point 与原完整门真正关闭前，不再启动 compiler optimization。固定 archive 只能重建精确 `DBC154…` C，无法重建 `5E862…` / `9DFD…` native-object pins，且没有另存的权威 object/recipe；因此 22 GiB crossing 路线已永久关闭且不计为已执行。当前顺序为 `I-prime -> A-prime atomic transfer authority -> S-prime cleanup -> final acceptance`：behavior-preserving exact-slot identity 层 I-prime 已完成；source-built 反例已否决完整 S-prime 独立于 A-prime 的旧假设。下一门必须先原子贯通 executable-HDecl inventory、transitive may-Drop、callable ownership modes、CFG/显式 `Take` 与 source-clear，再恢复 exact-none NOOP producer lattice和 S-prime cleanup；ctor-name 特判、删除 ANF Drop、runtime dup、旧双遍历、unity 或资源加码均不得作为捷径。
...
> **S′ 独立性反证与 checkpoint 重排（2026-08-22）**：最新 main 的 source-built S′ gen1 对 direct/named owning payload 均生成“source 进入 owning sink 后仍在 block exit Drop”的同型 C，focused runtime/RC/structural 全红；旧 a11 S′ 绿色依赖祖先 A′ `HExpr::Take` 保存值并清 source slot，不能作为独立 S′ 证据。实验 admission 层已可恢复撤回，I′ 完整保留。当前下一且仅下一 checkpoint 为 A′ atomic transfer authority：一次贯通 checker→HIR→Perceus→verifier→codegen 的 executable-HDecl inventory、transitive may-Drop、callable modes、CFG/显式 `Take`/source-clear；transfer canary 通过后才恢复 exact-none W4/exit、tail negatives、shadow/loop/catch。不得把 fresh-only ANF 子集、Option ctor 特判或局部 verifier 补丁冒充该 authority。
