...
> **Approved A3 final delayed-hydration crossing**：保留original direct/full `graph.dependencies` list、membership、顺序与pre-inference injection完全不变；39/61 module的已观察topo reorder不得再出现。另构造non-direct prior-topo shallow physical list并以显式额外参数传入`check_module`。只有在inference/effect facts/impl validation/dict lowering全部完成后、`freeze_core_and_legacy_facts`之前，late helper才遍历TypeDefs、exact-owner dedupe、调用现有localizer并插入type registries；不publish field effect schemas，不处理其他metadata，不改source namespace或永久IR/ABI。

> A3固定actual ABI17/typeid745、exact83与480-types零Fn/open-tail/schema-tail/non-direct-Drop/extern census、canonical owner/edge单射及61/61 direct order。Generated-C patch上限为260 formatted LOC且少于50 semantic actions，只触projection helper/separate list、`check_module`显式参数与late helper/hook，unexpected diff 0且RC独立review CLEAR后才可运行。所有临时artifact hash固定且非candidate/fallback；只有clean second product进入fixed point/smoke。A3为最后一次bridge尝试：任一preflight/运行失败、缺metadata/identity、额外C/source edit或重复panic均立即stop，不允许A4，后续只在B1/B2间重新由用户裁决。
