...
> **Approved topo-prefix generated-C crossing**：固定clean `d06 main.c`只允许在`compiler_mod::compile_phases`做reviewed三语句patch，把每module的direct `r_deps`替换为`graph.topo_order`的deterministic borrowed copy；既有`module_exports_map`对future/current key自动skip，实际得到prior-topological metadata prefix。Patch不改变source-visible namespace、resolver或source semantics，也不新增name fallback、alias或第二registry。Patched seed只编译上述overlay bridge；bridge只编译clean private current source；只有clean second product可进入gen2/gen3 fixed point与smoke。所有blob/source/patch/manifest/tool/output hash固定，patched seed/overlay/bridge不commit、不进tracked anchor、不作candidate/fallback，crossing后永久删除；失败或任何额外generated-C修改立即stop并重新请求用户决定。
