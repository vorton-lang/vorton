...
> **Builtin `Eq.ne` clean break（2026-08-30 用户批准 C）**：#268/#269 的 builtin public `Eq` inventory只保留exact `eq` member；`!=`唯一降低为`!exact Eq.eq`。删除`Eq.ne`、primitive Ne intrinsic、Option/derived Ne slot或body、default-specialization及其manifest/oracle，不实现sibling evidence rebase或无consumer样板。一般source trait default method及其Core/Flow/ABI验收保持，不因本项回滚。
...
> **Generic Physical RC B-min conditional fallback（2026-08-30 用户批准设计并暂停执行）**：撤销旧“禁止Ptr/extern作为generic actual”候选；#268/#269仍必须保留`List/Map/Option/Box<Ptr|extern>`等0.1 consumer。Outer aggregate shell与direct payload policy分轴：shell/buffer始终release（immortal`Option.none`例外），每个storage formal/field只携`NoRc/RingRc` physical policy，Logical Unique与Borrow/Own/Clone/Take仍由ONE Planner静态裁决；删除whole-value foreign cleanup veto。Hidden evidence只服务真实retain/release/store-for-later obligation，escaping dependent shell存packed mask，identity/pass-through/HOF与无义务generic零evidence；certificate/verifier分别证明shell owner与payload policy。**当前production/runtime/codegen/验收冻结**，先完成用户批准的full reachable monomorphization feasibility并裁决M vs shared-context：若concrete实例全部特化，policy常量化且不得实现mask；仅M拒绝或证明仍有shared generic ABI边界时，恢复该边界的最小B-min。禁止双实现、name/header sniff、function table、第二solver或借机前移D+B/B-152；本暂停不降低Ptr generic consumer与最终RC/ASan门，也不新增post item。

> **Struct move-spread partial/open-drop gate（2026-08-30 用户批准）**：#268/#269 的 struct move spread 必须始终构造 fresh result；字段静态类型保持 `T`，Flow / ONE Planner 以 exact field path 的 `Live` / `Moved` occupancy 作为唯一初始化状态 authority。字段 exact Take 后 source slot 清为 physical empty / NULL；普通 aggregate Drop 只 Drop `Live` 字段、跳过 empty / `Moved` 字段并恰好一次 release shell。全 shareable 字段走 physical RC dup、base 保持完整后执行同一普通 Drop。禁止 whole-base MovePlace / storage reuse、`ReleaseMovedAggregate`、shell bypass、extra whole Drop、把 occupancy 编码成公开 `Option<T>` / `MaybeUninit<T>` 或 backend 重建。直接 user-`Drop` aggregate 若 spread 需要任一 partial Take，必须在 source/TypedHIR 边界稳定拒绝；不得把 partial `Self` 交给用户 destructor。验收必须覆盖 base 单次求值、override LTR 且失败前零partial move、Take后source-clear、mixed Live/Moved open drop、shareable physical RC dup、shell恰好释放一次、user-Drop负例，以及杀死未clear、double-drop和专用shell释放路径的mutation。本门只约束struct/aggregate；named-enum update的支持边界继续冻结，不在本决定中推断。
