...
**一次性Python native inline-module SCU bootstrap例外（2026-08-30 用户批准）**：固定clean authority的62个compiler Ring files中只包含61个reachable files（main + 60 dependency sources），`perceus.ring`是唯一orphan；固定DAG为433条unique dependency edges、0 unresolved/cycles。仓库外generator按dependency-first topo与lexicographic tie-break，把60个dependency sources原样包进Ring原生inline `pub mod` frame，只把每个wrapper零缩进的sibling `use/pub use` root改成`super::`，并以`fn main(){ cli::cli_main() }`替换root main；不得重命名declaration/reference leaves、模拟原文件`$$_`identity、修改strings/ABI/intrinsic或改变算法。Preserved `58075b40`只作静态donor（script blob `dafb14b6…`、test blob `0c2807c0…`），必须修正为exact reachable include list、排除`perceus`、上述稳定拓扑、system `%TEMP%` enforcement及新的manifest/census/hash receipt后才可使用。

首次d06命令前，root必须按§4.3.4固定source/tree、generator/script、manifest、生成flat source与observed stage的EvidenceKey，证明60 wrappers/433 edges/entry/import rewrite totality、unexpected source edits=0并取得独立review。Script、wrapper source、生成C与bridge executable全部hash-pinned、temp-only、非candidate/commit/fallback；bridge只能编译完全未修改的clean current project，只有该clean second product可进入gen2/gen3 fixed point与compiler/hello smoke。Root-owned hypothesis H1是“单一InferCtx/TypeEnv消除跨`ModuleExports` physical metadata合并”；expected first falsifier为fixed-artifact d06 single-file `check`。若需要leaf/global renaming、old module identity/string/ABI仿真、compiler/resolver或tracked source修改、第二authority或预计投入`>=10 active h`，立即stop回用户；不得自动转pub/backport/B2，也不恢复generated-C bridge。

**H1 review/launch concurrency override（2026-08-30 用户决定）**：仅对上述首个fixed d06 single-file check，runner/plan的read-only review允许与exact resource-capped命令并发；fixed tuple为authority `05acf602…` / compiler tree `51f1763d…`、flat `F61405E6…`、runner `9C490323…`、plan `2E89DCB1…`、d06 `D2078ED3…`，命令与limits只取plan。并发结果必须quarantine，在exact runner/plan review CLEAR且root对post-run EvidenceKey重新核验前，不得支持H1、build、bridge、fixed point、bookkeeping、下一命令或任何claim。Review BLOCK时无论命令exit为何都立即invalidate/discard结果；不得retry或fallback。该override只改变review/launch顺序，不改变artifact tuple、H1、first falsifier、postcondition、安全门或no-retry纪律。
