...
**一次性Python flat-source SCU bootstrap例外（2026-08-30 用户批准）**：为避免在0.1迁仓检查点前插入数十小时carrier backport/replay，允许对固定clean authority源码运行一个仓库外、可丢弃的Python source flattener：按确定性module拓扑拼接compiler Ring源码，将module级function/const/type/trait/effect/extern等声明及其真实global引用改写为全局唯一identifier，展开或删除compiler-internal import/alias/re-export，并保留一个明确entry `main`。这是既有single-file checker的输入生成，不是compiler SCU模式、module语义变更或C translation-unit工作；tracked compiler/runtime/std/tests源码保持不变。

生成器必须token/scope-aware：strings、comments、interpolation文本、locals/params/type params/pattern binders、field/method/variant叶子不得被误改；extern ABI、runtime/prelude/`$builtin`/compiler-intrinsic spelling必须原样。运行前manifest须证明每个原module-level declaration恰有一个mapping、每个internal imported/global use恰有一个target、flat duplicate/unresolved internal use为0，entry/extern/intrinsic census exact，且独立review CLEAR。Script、mapping、flat Ring、生成C和bridge executable全部hash-pinned、temp-only、非candidate/commit/fallback；可在首次build前修正同一机械生成器，但不得删除声明、改算法、绕过type/effect/ownership检查、修改generated C、建立name fallback/第二registry或写tracked source。只有d06既有single-file check/build产出的bridge可编译完全未修改的clean current project，且只有该clean second product可进入gen2/gen3 fixed point与compiler/hello smoke。若静态事实要求compiler/resolver新功能、module-identity仿真、string/ABI rewrite、tracked source修改、第二authority或预计投入达到10 active hours，立即stop并回用户；不得自动转pub/backport/B2，也不恢复任何generated-C bridge。
