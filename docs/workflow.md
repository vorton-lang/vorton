...
**5d57 0.1 release-cut纠偏（2026-08-31 用户决定，supersede上述live终点）**：分阶段历史构建已取得最后一个source-build成功rung `5d576f7cd34a015935f67932c3136fd22d01fb65`。继续要求current-tip被旧代compiler理解，会把普通type/effect/IR修复扩大为代际兼容工程；0.1因此固定clean 5d57源码做同源gen2/gen3 `main.c`文本fixed point与compiler/hello smoke，不再把`04b3`及其后续current-tip自举作为#268/#269门。5d57之后提交与失败证据完整保留为post-cut WIP；不得为把它们塞回0.1恢复B1/direct-edge/publicization/SCU/generated-C或新的compatibility bridge。迁仓后先建立外部宿主语言compiler基础，语言语义、IR与runtime契约稳定后再恢复self-host里程碑。
