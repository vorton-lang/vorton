...
Governance CI 在 Ubuntu 上依次运行 current-tree 结构检查、Rust format、Clippy、workspace tests 与 Git whitespace gate，并以独立 job 运行所需的 Verus／Proptest 工具自检。精确命令以 [workflow](../.github/workflows/test.yml) 为准，工具边界见[外部验证工具链](validation-toolchain.md)。
