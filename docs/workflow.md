...
**一次性A′ ABI17 types/layout-only bootstrap例外（2026-08-30 用户批准）**：固定clean d06的`ModuleExports`必须保持17 args/17 pointer slots/typeid745，constructor/drop/register与matching source layout完全一致。Direct dependencies继续使用原完整exports；只对non-direct prior-topo entry构造shallow projection，dup并保留slot 0 `module_key`、1 `module_prefix`、5 `types`、13 `struct_field_orders`，slots 2–4、6–12、14–16全部使用fresh empty collection，特别是slot 6 `type_aliases`、7 `effects`、8 `effect_aliases`、9 `traits`、10 `trait_impls`、11 `method_index`、12 `inherent_methods`不得运输。Helper只复用既有17参数constructor与typeid745 ownership：4次dup、13个fresh empty转交constructor、`List.push`后local projection drop，最终drop释放0..16。Source namespace仍唯一由`namespace_plan`裁决，不新增name fallback、alias、registry、source语义或永久carrier。

A′只服务既有exact83 overlay；helper上限24 semantic actions，unexpected diff必须为0，全部seed/blob/source/helper-patch/manifest/tool/output hash固定。Patched seed只编译overlay bridge，bridge只编译clean private current source，只有clean second product可进入gen2/gen3 fixed point与compiler/hello smoke。Patched seed/helper/overlay/bridge不得commit、进入tracked anchor、成为candidate/fallback或复用为后续seed，crossing后永久删除。Constructor/drop/register/source mapping不一致、再次出现raw-tail panic、需要alias/value/trait/impl/Drop、canonical collision、private83 mismatch、helper residue、超action上限、任何额外C/source edit或其他失败时立即stop并重新取得用户批准；不得静默扩projection。本例外之外的generated-C compatibility shim继续受standing decision禁止。

**一次性A′ types/layout-only bootstrap例外（2026-08-30 用户批准）**：固定clean d06的`ModuleExports`必须保持17 args/17 pointer slots/typeid745，constructor/drop/register及matching source layout完全一致。Direct dependencies继续使用原完整exports；只对non-direct prior-topo entry构造shallow projection，dup并保留slot 0 `module_key`、1 `module_prefix`、5 `types`、13 `struct_field_orders`，slots 2–4、6–12、14–16全部使用fresh empty collection，特别是slot 6 `type_aliases`、7 `effects`、8 `effect_aliases`、9 `traits`、10 `trait_impls`、11 `method_index`、12 `inherent_methods`不得运输。Helper只复用既有17参数constructor与typeid745 ownership：4次dup、13个fresh empty转交constructor、`List.push`后local projection drop，最终drop释放0..16。Source namespace仍唯一由`namespace_plan`裁决，不新增name fallback、alias、registry、source语义或永久carrier。
