...
The first Checker subset applies `set.parameter_types`, `set.return_type`, fixed Borrow/Move `set.parameter_modes`, explicit empty `set.effect_upper`, and explicit empty `set.generic_requirements`. Types are limited to `Int`, `Float`, `Bool`, `Unit`, `Never`, tuples of those types, and references to usable non-generic transparent aliases. Omitted clauses remain omitted. Partial records for the same exact target merge idempotent normalized values and reject conflicts rather than using document order as last-wins.

All other target/type/mode forms, non-empty effect or requirement rows, `parameter_escape`, every `check` clause, and non-empty record `type_parameters` produce `CheckDiagnosticKind::Unsupported`. If one record selects both supported and unsupported clauses, the entire record is rejected before any supported fact can affect checking. Binding failures and conflicting selected facts use their own stable diagnostic categories. These diagnostics identify the actual document index and JSON path and may relate a real source `OriginRef`; they do not invent contract byte positions, source spans, or libraries.
